...
const log = (category, message) => console.log(`[${category}] ${message}`);

console.log(`Release docs verify passed for vscode version ${extensionVersion}.\n`);
log('registry', `vscode.versioning.current = ${moduleVersion}`);
log('registry', `vscode-marketplace artifact version = ${vscodeArtifact.version}`);
log('registry', `CAPABILITY_REGISTRY.md version = ${registryVersionLine[1]}`);
log('deploy',   'release:verify:docs command reference');
log('deploy',   'manual-only Marketplace publish policy');
log('deploy',   'keyboard shortcut override section + snippet');
log('deploy',   'vim/jetbrains/mac profile recommendation notes');
log('deploy',   'conflict matrix table with FAQ anchors and template links');
log('deploy',   'profile template open commands (generic + OS variants)');
log('deploy',   'code/code-insiders decision rule');
log('deploy',   'Linux shell variant commands + shell-family note');
log('deploy',   'code-insiders fallback commands (OS variants + compact block)');
log('deploy',   'compact copy block for code commands');
...
log('template', 'keybindings.jetbrains.json (next/previous bindings)');
log('template', 'keybindings.mac.json (next/previous bindings)');
log('lint',     'copy-path consistency (profiles x OS variants x CLIs)');
